...
Send **one complete message** per task. After one intended Send activation, do not activate Send a second time for that ID. **Sent** means the URL changed from the new-chat page to a conversation content page (`https://chatgpt.com/c/...`) **and a new user turn appears**; the assistant turn may still be generating. A cleared composer or Stop button alone is insufficient. If the page **remains at `https://chatgpt.com/`**, Owner has confirmed the message did not reach GPT: treat it as a **pre-Send failure**, clear the draft in that same tab, read back an empty composer, then release the unsent reservation and retire the ID. `mark-uncertain` applies **only when the page reached a conversation content URL but the turn state cannot be determined**; investigate read-only and never resend that ID. No web follow-up and no new ID to resend the same work after it reaches GPT.
...
Pre-Send mistakes, including a wrong card path or a control not yet loaded, are **not** follow-ups to GPT: correct and recheck before activating Send. A released reservation retires its ID, so the next authorized trial uses a new ID. If an **older ledger version** recorded a new-chat click as `uncertain_submission`, do not edit state by hand: after Owner confirms the new-chat rule and Controller clears/reads back the exact draft, hidden `correct-unsent --task ID --observed-url https://chatgpt.com/ --evidence NOTE --owner-decision NOTE` changes it to `web_not_sent`, restores local capacity, and keeps the ID retired. This correction cannot apply to a recorded Send or a conversation content URL. On deadline, `finish-task --timed-out --fallback-agent NAME --reason REASON` voids the ID immediately. A result committed afterward is recorded only with `late-result`; never adopt, merge, or count it as success.
...
2. Open **one foreground** Codex IAB tab to a fresh non-Project ChatGPT conversation. A zero-tab IAB is a normal start; the *possibility* of login is not a reason to stop before opening. Confirm the new-chat URL and zero message turns; do **not** require a legacy Chat radio control. If actual login, verification, suspicious activity, or an unrelated draft appears, follow §5. Let the page settle and reread it if the tier pill is missing from the first accessibility state; it can appear later. If the pill initially says `中`, open it, inspect `思考强度` and nested `选择模型`, set the far-right Pro strength, and verify the final composer label says **`6 Pro`**. Do not infer the model family from `中` alone. Check limit/downgrade and “已使用记忆” indicators. Paste the [single starter template](#6-web-gpt-starter-template), verify task ID, immutable card SHA, branch, text, final `6 Pro`, and exactly one ChatGPT tab. Distinguish the **actual Send button** (`ID: composer-submit-button`, the blue up-arrow inside the composer's lower-right corner) from the separate **“Add to Project” suggestion below the composer**. After pasting, get a fresh accessibility state **and screenshot**; AX numeric indices can change when the suggestion/layout appears. If any overlay is present, dismiss it before sending and reread. Activate Send **once** by the freshly located button; if that target remains ambiguous, focus the composer and press **Return once instead** as the one Send action. Do not use Return after an attempted button activation on the same ID. WCS-25's click left the page at new chat and surfaced a project suggestion; the CUA return did not identify the low-level hit node, so do not claim it definitely clicked the suggestion.
3. After the one action, inspect URL **and** turns. Conversation content URL plus new user turn permits hidden `mark-sent RESERVATION_UUID --url CONTENT_URL`; use the UUID printed by `reserve`, **not the task ID**. If the URL is still the new-chat root, clear and read back the composer empty, then hidden `release RESERVATION_UUID` and close the tab: this is pre-Send failure even when a button click was attempted. If the URL is a conversation content page but the user-turn state is unclear, hidden `mark-uncertain RESERVATION_UUID --reason CODE --url CONTENT_URL` and stop without a second Send. If cleanup cannot be verified, retain the reservation and report the blocker; do not silently free the slot. A previously existing unrelated draft is preserved, not cleared. After a confirmed Send, **keep the tab open** and mark it for handoff if the browser tool requires it.
4. Keep the Controller turn active and **continue the Controller's normal work after Send**. This web task is outside the critical path; never stop construction merely to wait for it. About every five minutes, **at a natural pause**—for example while checking CI, making a checkpoint, or switching work packages—also poll the web task. Each poll first reads the real ledger `status` and stops if the task has ended or a cooldown is active; then it inspects the **same ChatGPT tab read-only** and queries the assigned GitHub branch/result path read-only. The five-minute interval is a target for opportunistic checks, not a reason to interrupt useful work. **Only when the Controller genuinely has no other useful work** may it wait solely for this task, checking about every five minutes. Do not start a watcher, background GitHub process, queued notification, Codex hook, or separate reminder mechanism. GPT retrying a GitHub write while it is still generating is normal; a temporarily unchanged answer is not a reason to resend. A page error, stopped generation with no further actions, expired connector authorization, limit, or downgrade is an interruption. **Do not click Retry, Continue generating, or any other button**; record failure, keep evidence, and hand the work to Codex. Report login loss explicitly. A connector write-confirmation prompt is handled under §5, not clicked. If there is no result at the task deadline, use `finish-task --task ID --timed-out --fallback-agent NAME --reason REASON` and give the work to Codex; any later GitHub result is `late-result` only. Update Owner only at confirmed Send, verified GitHub result, or abnormal stop, not for unchanged polls.
5. When the Controller's poll finds a result, verify the real ledger and GitHub branch, exact commit/parent, allowed diff, result content, reviewed SHA, and **real Actions logs showing >0 executed tests**. Compare remote `main` to its pre-dispatch SHA; an unexpected commit stops this route and is reported to Owner. Read any reply-served-model location without clicking retry. Only **after** GitHub result confirmation, close that task's ChatGPT tab and verify the IAB inventory. Then hidden `approve-result --task ID --commit SHA` and `finish-task --task ID --result-commit SHA`. A GPT claim alone is not acceptance. A tab that disappears before inspection leaves model identity and explicit post-result close unverified.
...
No separate OS window may appear on Owner's screen, including a brief console. Browser work stays inside the Codex foreground IAB tab. Invoke `ledger.py` only through `.agents/skills/web-chatgpt-subagent/scripts/Invoke-LedgerHidden.ps1`, resolved locally from repository root. It starts `pythonw.exe` with `CreateNoWindow=true` and `WindowStyle=Hidden`; require `exit_code=0` and `console_window_present=false`. GitHub checks are read-only calls from the Controller's existing tool session, with no separately launched visible console. Do not run `python.exe ledger.py`, a visible `Start-Process`, a background watcher, `codex queue`, or a notification hook. Store private ledger receipts under `%LOCALAPPDATA%\gogoke\web-chatgpt-subagent`; no user-profile absolute path belongs in the public task card.
