...
4. Start the GitHub watcher using §5's hidden wrapper, preserve the conversation tab, then **end the Controller turn**. Do not keep the turn active with `sleep` or short waits: watcher `codex queue` notices are delivered after the turn ends. The watcher checks the assigned GitHub branch about every 90 seconds, then every five minutes after 20 minutes; on finding a result it immediately queues a result notice, then **repeats that notice every 30 seconds until the Controller acknowledges the exact candidate SHA**. It also queues a **read-only page-patrol reminder about every five minutes** while no result has appeared. On a patrol notice, the Controller reads the **same tab once**, takes no page action, records a real interruption if present, then ends that turn. GPT retrying a GitHub write while it is still generating is normal; a temporarily unchanged answer is not a reason to resend. A page error, stopped generation with no further actions, expired connector authorization, limit, or downgrade is an interruption. **Do not click Retry, Continue generating, or any other button**; record failure, keep evidence, and hand the work to Codex. Report login loss explicitly. A connector write-confirmation prompt is handled under §5, not clicked. On result notice, read `watch-{TASK_ID}.json` in `%LOCALAPPDATA%\gogoke\web-chatgpt-subagent`, write its `head_sha` (only that SHA) to `watch-{TASK_ID}-ack.txt` in the same directory with native PowerShell `Set-Content -NoNewline`, then independently verify ledger/GitHub/CI under step 5. This acknowledgement means **the Controller was awakened**, not that the result passed. If notification delivery fails or the watcher reaches deadline, inspect its receipt once for candidate, notification error, or deadline expiry; avoid model-driven GitHub polling. Update Owner only at confirmed Send, verified GitHub result, or abnormal stop, not for unchanged patrols.
