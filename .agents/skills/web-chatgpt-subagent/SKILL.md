...
3. After the one action, inspect URL **and** turns. Conversation content URL plus new user turn permits hidden `mark-sent ID --url CONTENT_URL`. If the URL is still the new-chat root, clear and read back the composer empty, then hidden `release ID` and close the tab: this is pre-Send failure even when a button click was attempted. If the URL is a conversation content page but the user-turn state is unclear, hidden `mark-uncertain ID --reason CODE --url CONTENT_URL` and stop without a second Send. If cleanup cannot be verified, retain the reservation and report the blocker; do not silently free the slot. A previously existing unrelated draft is preserved, not cleared. After a confirmed Send, **keep the tab open** and mark it for handoff if the browser tool requires it.
