...
class DispatchSafetyTests(unittest.TestCase):
    ...
    def test_one_active_web_task_even_after_reply_until_github_result(self):
        ...
        early_finish = self.run_ledger("finish-task", "--task", "one", "--result-commit", "a" * 40)
        self.assertEqual(early_finish.returncode, 2)
        self.assertIn("approve", early_finish.stderr)
        self.assertEqual(self.run_ledger("approve-result", "--task", "one", "--commit", "a" * 40).returncode, 0)
    ...
    def test_f3_same_task_id_cannot_be_reserved_twice_across_seats(self):
        ...
        self.assertEqual(self.run_ledger("mark-sent", winner.stdout.strip(), "--url", "https://chatgpt.com/c/example").returncode, 0)
        self.assertEqual(self.run_ledger("complete", winner.stdout.strip(), "--actual-tier", "high").returncode, 0)
        self.assertEqual(self.run_ledger("approve-result", "--task", "same-id", "--commit", "a" * 40).returncode, 0)
    ...
    def test_f3_task_id_survives_state_loss_and_reconciliation(self):
        ...
        self.assertEqual(first.returncode, 0, first.stderr)
        self.assertEqual(self.run_ledger("mark-sent", first.stdout.strip(), "--url", "https://chatgpt.com/c/example").returncode, 0)
        self.assertEqual(self.run_ledger("complete-unverified", first.stdout.strip()).returncode, 0)
        self.assertEqual(self.run_ledger("approve-result", "--task", "old-id", "--commit", "a" * 40).returncode, 0)
        self.assertEqual(self.run_ledger("finish-task", "--task", "old-id", "--result-commit", "a" * 40).returncode, 0)
        self.state.unlink()
        self.assertEqual(self.run_ledger("reconcile", "--source", "controller starts new baseline; prior unknown").returncode, 0)
        denied = self.run_ledger("reserve", "--tier", "medium", "--task", "old-id", "--seat", "b")
        self.assertEqual(denied.returncode, 2)
        self.assertIn("task ID already used", denied.stderr)
        fresh = self.run_ledger("reserve", "--tier", "medium", "--task", "fresh-id", "--seat", "b")
    ...
    def test_confirmed_or_uncertain_send_cannot_be_released_for_retry(self):
        ...
        self.assertEqual(self.run_ledger("approve-result", "--task", "sent", "--commit", "a" * 40).returncode, 0)
