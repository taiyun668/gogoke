...
gogoke is a Tauri app for orchestrating multiple Codex agents across local workspaces. It provides a sidebar to manage projects, a home screen for quick actions, and a conversation view backed by the Codex app-server protocol.

The application, telemetry boundary, release channel, package identity, and update trust root are owned by gogoke. The imported source provenance and MIT notice are recorded in `THIRD_PARTY_NOTICES.md` and `LICENSE`; no runtime follows that source repository.
...
2. On desktop gogoke, open `Settings > Server`.
...
6. On iOS gogoke, open `Settings > Server`.
...
```bash
cd src-tauri
cargo build --bin gogoke_daemon --bin gogoke_daemonctl
```
...
./target/debug/gogoke_daemonctl status

# Start daemon using host/token from settings.json
./target/debug/gogoke_daemonctl start

# Stop daemon
./target/debug/gogoke_daemonctl stop

# Print equivalent daemon start command
./target/debug/gogoke_daemonctl command-preview
```
...
- `--daemon-path <path>`: explicit `gogoke_daemon` binary path
...
Windows releases use gogoke's own GitHub release channel. CI produces an unsigned NSIS installer, portable ZIP, and `SHA256SUMS.windows`. Before publishing a release, the Owner signs that manifest offline with `../../tools/sign-gogoke-release-manifest.ps1`; the private key remains outside the repository and CI. Installed copies offer an update only after the exact installer name, version, manifest signature, and SHA-256 all agree. The update coordinator waits for the old process, replaces a clean product directory, requires a version-bound readiness receipt, and rolls back on installer or startup failure.
...
src-tauri/
  ...
  src/bin/gogoke_daemon.rs  remote daemon JSON-RPC process
  src/bin/gogoke_daemon/rpc/  daemon RPC domain handlers
...
- The remote daemon entrypoint is `src-tauri/src/bin/gogoke_daemon.rs`; RPC routing lives in `src-tauri/src/bin/gogoke_daemon/rpc.rs` and domain handlers in `src-tauri/src/bin/gogoke_daemon/rpc/`.
