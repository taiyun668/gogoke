...
This fork includes a **proof-of-concept** daemon that runs gogoke's backend logic in a separate process (intended for WSL2/Linux), exposing a simple **line-delimited JSON-RPC** protocol over TCP.
...
cargo run --bin gogoke_daemon -- \
  --listen 127.0.0.1:4732 \
  --data-dir "$HOME/.local/share/gogoke_daemon" \
  --token "$TOKEN"
```
