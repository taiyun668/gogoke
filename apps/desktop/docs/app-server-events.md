...
- Which app-server events gogoke supports right now.
- Which app-server requests gogoke sends right now.
- Where to look in gogoke to add support.
...
3. Compare Codex events vs gogoke routing.
4. Compare Codex client request methods vs gogoke outgoing request methods.
5. Compare Codex server request methods vs gogoke inbound request handling.
6. Update supported and missing lists below.

Related project skill:
- `.codex/skills/app-server-events-sync/SKILL.md`

## Where To Look In gogoke
...
These are the current Codex v2 `ServerNotification` methods that gogoke
...
## Additional Stream Methods Handled In gogoke
...
- `codex/backgroundThread` (gogoke synthetic bridge event)
- `codex/connected` (gogoke synthetic bridge event)
...
## Supported Requests (gogoke -> App-Server, v2)

These are v2 request methods gogoke currently sends to Codex app-server:
...
Compared against Codex v2 request methods, gogoke currently does not send:
...
## Server Requests (App-Server -> gogoke, v2)
...
4. List gogoke outgoing requests:
...
6. Check for camelCase vs snake_case mismatches:
   ...
   - gogoke generally defends against this by checking both forms (for example in `threadNormalize.ts` and `useAppServerEvents.ts`), while centralizing method/type parsing in `appServerEvents.ts`.
...
- Steering behavior while a turn is processing:
  - gogoke attempts `turn/steer` only when steer capability is enabled, the thread is processing, and an active turn id exists.
  - If `turn/steer` fails, gogoke does not fall back to `turn/start`; it clears stale processing/turn state when applicable, surfaces an error, and returns `steer_failed`.
