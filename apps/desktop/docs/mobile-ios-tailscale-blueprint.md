# gogoke iOS Remote Blueprint (Tailscale + TCP)

This document is the canonical runbook for iOS remote usage with a desktop-hosted gogoke backend over Tailscale.
...
1. Desktop gogoke hosts the daemon and executes Codex workflows.
2. iOS gogoke connects to the desktop daemon using `remoteBackendHost` + token.
...
- Desktop gogoke is installed and able to run local workspaces.
