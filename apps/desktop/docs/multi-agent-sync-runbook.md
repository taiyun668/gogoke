...
Keep gogoke's Agents settings behavior in sync with upstream Codex (`../Codex`) whenever Codex changes:
...
- When users report a mismatch between gogoke settings and Codex runtime behavior
...
## gogoke Files To Update If Upstream Changes
...
- `src-tauri/src/bin/gogoke_daemon/rpc/codex.rs`
...
- Keep gogoke scoped to the new key only (no legacy alias read/write).
- Keep gogoke writes aligned with upstream expectations.
...
- Reconcile gogoke guardrails when upstream changes.
...
- gogoke default `agents.max_depth` is `1`.
- gogoke currently enforces a product cap of `12` for `agents.max_threads` and `4` for `agents.max_depth` in UI + backend.
