...
#[path = "../storage.rs"]
mod storage;
#[path = "gogoke_daemon/transport.rs"]
mod transport;
#[allow(dead_code)]
#[path = "../types.rs"]
mod types;
...
const DAEMON_NAME: &str = "gogoke_daemon";
...
fn default_data_dir() -> PathBuf {
    if let Ok(xdg) = env::var("XDG_DATA_HOME") {
        ...
        if !trimmed.is_empty() {
            return PathBuf::from(trimmed).join("gogoke_daemon");
        }
    }
...
fn usage() -> String {
    format!(
        "\
USAGE:\n  gogoke_daemon [--listen <addr>] [--data-dir <path>] [--token <token> | --insecure-no-auth]\n\n\
...
mod tests {
    ...
    fn make_temp_dir(prefix: &str) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let dir =
            std::env::temp_dir().join(format!("gogoke-{prefix}-{}-{unique}", std::process::id()));
    ...
    fn test_state(data_dir: &std::path::Path) -> DaemonState {
        ...
        DaemonState {
            ...
            codex_login_cancels: Mutex::new(HashMap::new()),
            daemon_binary_path: Some("/tmp/gogoke_daemon".to_string()),
        }
    }
...
fn main() {
    ...
    runtime.block_on(async move {
        ...
        eprintln!(
            "gogoke_daemon listening on {} (data dir: {})",
            config.listen,
            state
                .storage_path
                .parent()
