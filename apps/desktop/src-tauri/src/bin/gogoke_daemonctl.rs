...
const APP_IDENTIFIER: &str = "app.gogoke.desktop";
...
async fn daemon_start(
    ...
    if !insecure_no_auth && token.is_none() {
        return Err("Authenticated loopback TCP requires a Remote backend token.".to_string());
    }
...
async fn daemon_stop(listen_addr: &str, token: Option<&str>) -> TcpDaemonStatus {
    ...
    match probe_after_stop {
        ...
        DaemonProbe::NotReachable => TcpDaemonStatus {
            ...
            started_at_ms: None,
            last_error: Some(stop_error.unwrap_or_else(|| {
                "Daemon status is unknown; no authenticated managed daemon ownership was established, so no process was terminated."
                    .to_string()
            })),
            listen_addr: Some(listen_addr.to_string()),
        },
...
async fn daemon_status(listen_addr: &str, token: Option<&str>) -> TcpDaemonStatus {
    ...
    match probe_daemon(listen_addr, token).await {
        DaemonProbe::Running {
            ...
            auth_error,
            info,
        } if auth_ok && info.as_ref().is_some_and(is_managed_daemon) => TcpDaemonStatus {
            state: TcpDaemonState::Running,
            pid,
            started_at_ms: None,
            last_error: None,
        ...
        DaemonProbe::Running {
            ...
            pid,
            started_at_ms: None,
            last_error: Some(auth_error.unwrap_or_else(|| {
                format!(
                    "Refusing daemon status because authenticated managed identity could not be verified: {}",
                    info.map(|value| format!("name={}, mode={}, version={}", value.name, value.mode, value.version))
                        .unwrap_or_else(|| "identity metadata unavailable".to_string())
                )
            })),
            listen_addr: Some(listen_addr.to_string()),
        },
...
mod tests {
    use super::{
        can_force_stop_daemon, daemon_connect_addr, daemon_listen_addr, local_listener_port,
        parse_netstat_listener_pid, parse_port_from_remote_host, parse_ss_listener_pid,
        resolve_listen_addr, safe_force_stop_pid, shell_quote, DaemonInfo,
    ...
    fn builds_listen_addr_with_fallback_port() {
        ...
        assert_eq!(daemon_listen_addr("mac.example.ts.net"), "127.0.0.1:4732");
    }
    ...
    #[test]
    fn force_stop_requires_authenticated_full_identity() {
        let base = || DaemonInfo {
        ...
        assert!(can_force_stop_daemon(true, Some(&base())));
        assert!(!can_force_stop_daemon(false, Some(&base())));
...
LISTEN 0      4096   0.0.0.0:4732      0.0.0.0:*    users:(("gogoke-da",pid=12345,fd=7))
"#;
...
tcp        0      0 0.0.0.0:4732            0.0.0.0:*               LISTEN      6789/gogoke-da
"#;
