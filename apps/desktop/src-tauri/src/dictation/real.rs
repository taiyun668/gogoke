...
pub(crate) async fn dictation_download_model(
    ...
    release_policy::require_voice("dictation model download")?;
    let model_id = resolve_model_id(&state, model_id).await;
...
pub(crate) async fn dictation_cancel_download(
    ...
    let owned_download = {
        let mut dictation = state.dictation.lock().await;
        if dictation.download_cancel.is_none() || dictation.download_task.is_none() {
            false
        } else {
            if let Some(flag) = dictation.download_cancel.take() {
...
pub(crate) async fn dictation_remove_model(
    ...
    release_policy::require_voice("dictation model removal")?;
    let model_id = resolve_model_id(&state, model_id).await;
...
pub(crate) async fn dictation_start(
    ...
    release_policy::require_voice("microphone capture")?;
...
pub(crate) async fn dictation_request_permission(app: AppHandle) -> Result<bool, String> {
    release_policy::require_voice("microphone permission request")?;
...
pub(crate) async fn dictation_stop(
    ...
    if !release_policy::voice_enabled() {
        return stop_owned_voice_activity(&app, &state).await;
    }
    let cancel_flag = Arc::new(AtomicBool::new(false));
...
async fn stop_owned_voice_activity(
    app: &AppHandle,
    state: &State<'_, AppState>,
) -> Result<DictationSessionState, String> {
    let owned_session = {
    ...
    if let Some(session) = owned_session {
        ...
        let _ = session.stopped.await;
        session.audio.lock().unwrap().clear();
    }
