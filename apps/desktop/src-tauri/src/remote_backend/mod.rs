...
mod tests {
    ...
    #[test]
    fn resolve_tcp_transport_keeps_authenticated_loopback_legacy_path() {
        let mut settings = AppSettings::default();
        settings.remote_backend_host = "127.0.0.1:4732".to_string();
        settings.remote_backend_token = Some("token".to_string());
        ...
        assert_eq!(host, "127.0.0.1:4732");
    }

    #[test]
    fn resolve_tcp_transport_rejects_external_host_before_connect() {
        let mut settings = AppSettings::default();
        settings.remote_backend_host = "tcp.example:4732".to_string();
        settings.remote_backend_token = Some("token".to_string());

        let error = resolve_transport_config(&settings).expect_err("external remote is sealed");
        assert!(error.starts_with("FEATURE_SEALED"));
    }
