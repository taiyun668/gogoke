...
- **旁聊要 fork 主控的上下文**，也就是项目的上下文。不 fork 就没法真的帮你解决问题。
  ...
  - **旁聊既可以持久保存，也可以丢弃；还能持续更新主控的上下文**：主控后来的进展会不断同步进旁聊，但同样只作参考，不执行其中的指令。（2026-09-26 Owner）
...
- **项目生命周期**：分最小实现模型阶段和正式阶段。每个阶段都是：需求采集与调研 → 设计大纲 → 审计 → 施工方案 → 审计 → 施工包 → 审计 → 施工 → 审计 → 合并 → 总审计 → 交付。（2026-09-26 Owner）
- **主控下辖的角色不能直接找你。**
