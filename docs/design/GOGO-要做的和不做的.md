...
- **不动 gogoke 现有 UI，只加。** 中顶、设置各节、面板三个 tab 一律保留。
  - **2026-09-26 Owner 澄清**：界面本身可以改，布局、组件、交互都能调整；**风格语言不能改**，也就是液态玻璃质感、配色、字体、圆角和间距体系都保持不变，不能把液态玻璃变成赛博朋克。组件能复用的复用，复用不了的就找风格相近的现成组件来改。
- **首版只做代码生产**，其余先不管。
...
- **引导按钮**：主控在长程任务中不会停下来。消息**先放进收件箱排队，再从收件箱里点"引导"**，不管主控忙不忙，都把这条消息直接插进它正在跑的回合。排队中的消息**可以取消，也可以编辑**；已经投递的不能收回。原生不支持插话的厂商，改为打断后带着这句话续接，并如实标明。（2026-09-26 Owner）
- **用户体验和交互细节一定要兼顾好。**每个机制都要把状态、可做的操作、如实显示、出错恢复、能否撤回都想全。收件箱的引导、取消、编辑只是一个例子。（2026-09-26 Owner）
- **工人也是席位**，长期还是短期由主控决定；功能上长期和短期两种机制都要有。只有主控能新建席位，工人不能再往下派。（2026-09-26 Owner）
- **工人的动态显示在项目的席位管理页里**，不跟主控、旁聊挤在一个界面。（2026-09-26 Owner）
- **Cursor 不进首版**，首版仍是五家：Claude Code、Codex、OpenCode、Grok Build、Antigravity。（2026-09-26 Owner）
- **主控不能自己决定自己用哪个实例**，由用户设定；主控只编排它下辖的席位。（2026-09-26 Owner）
- **席位管理至少分两层**：用户层（用户设定主控等席位，并给主控划编排范围：能用哪些实例、模型、推理强度、最多几个席位、权限上限）；主控层（主控在这个范围内编排下辖席位）。宿主强制执行边界，主控动不了用户层的席位。（2026-09-26 Owner）
- **秘书长、旁聊都能写**：往收件箱发消息、编辑、取消、引导都是写。写到什么范围由权限配置，宿主强制执行。（2026-09-26 Owner）
- **一个项目设几个主控、几个审计、还有什么角色，是产品化之后用户的用法**，现阶段不定。（2026-09-26 Owner）
- **额度要显示**：能用本地数据（CLI 自己在本机报出的额度信息）就用本地数据，用不了才调官方接口，并设好调用间隔防滥用。取代老 GOGO `31` §6 的"不显示剩余额度"。（2026-09-26 Owner）
- **进程以现状为准**：由 Rust native-host 起进程并放进 Job。整合设计 §11.2 的"由 seat-runtime（Node）起进程"作废。（2026-09-26 Owner）
- **只换实例，不换账号**：额度用完或需要换能力时，是给席位换一个实例（实例就是一个已登录的 CLI 账号），不在一个实例内部倒腾账号。（2026-09-26 Owner）
- **主控能自主编排下辖席位**：用什么角色、哪一家的哪个实例、什么模型、什么推理强度，都由项目主控自己决定。按什么流程、经过哪些计划和审计，是产品化之后用户的用法；**基础设施要保证主控有这个能力**。（2026-09-26 Owner）
- **实例管理页在设置里**：
  - 实例能不能重复登录，由每一家 CLI 自己决定，产品不另设规则；
  - **不自动升级 CLI**，只显示"有新版本"；
  - 这些都放在实例管理页里。（2026-09-26 Owner）
...
- **审阅设计稿 37 时补充的决定**（2026-09-26 Owner）：
  - 每个实例用自己独立的目录，在里面关掉厂商自带的记忆，不影响 Owner 自己在用的 CLI；记忆存在本机，不是跟着账号走的。
  - 席位总数不设上限；同时运行的席位数有上限，按本机资源来定。
  - 短期席位什么条件下回收，由主控自己定。
  - 主控接手项目时，必须先回答一组问题，才能开始派活。问题清单见设计 37 §2b1。
  - 上下文和记忆一定要隔离好。全局的那一份，要全局可用，又不能被项目里的检索查到。
  - 上下文里还有一层是旁聊。
  - 所有呈到 Owner 面前的问题，都要带解决方案，并给自由回答的选项。厂商自带问题卡的直接显示，没有的用我们自己的。
  - 写代码只在各自隔开的空间里进行，通过之后才能合并进主树。
