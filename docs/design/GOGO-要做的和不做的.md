...
- **审阅设计稿 37 时补充的决定**（2026-09-26 Owner）：
  ...
  - 写代码只在各自隔开的空间里进行，通过之后才能合并进主树。能不能合并靠上下文判断，不走机械关口；合并权限给谁，是产品化之后用户自己决定的用法。
  - 接手时必答的问题，同时是主控了解"这个项目空间怎么运作"的基础。
  - 上下文压缩和换会话很有必要，但不交给用户决定，要做到有这个动作、用户基本无感。
  - 仓库可以跨项目共享。仓库相关的机制（工作树、提交署名、凭证、哪些放仓库）照搬 Codex 和 Claude Code 的做法（设计 37 §2c）。
