...
候选公钥与 Owner 公钥都固定编进**同一份**稳定 shell；候选 sidecar 只在已登记的候选安装/测试域验签。Owner Win11 候选在 Owner 账户下运行，但使用与正式产品不同的安装根、HKCU 登记、数据根和安装实例 ID；候选验签入口在加载资源前核该实例与安装根，不能只靠命令行开关切换域。**资源包中的服务 JS 是可执行代码**，仍有同一账户的文件、网络与 `gh` 凭据访问能力；独立产品根不构成 OS 沙箱，native Product Authority 只约束受管入口，不能声称阻断任意 JS 直接 I/O。候选执行的信任前提是受信 CI 签名 workflow 的来源/产物限制、运行前 exact hash 核验和本机测试授权。R2-05 测试账本写入继续沿用现有 Owner `gh` 登录与受管通道，固定 `taiyun668/gogoke`、`s1-r4-ledger-test/r2-02`、`apps/desktop/test-fixtures/s1-r4/ledger/r2-02-results/`，保留 native 准入、journal 幂等与 Git 回读；不得写 main、正式产品账本或把测试结果称为 adoption/Goal Acceptance。该仓库/分支/路径限制属于**受管产品写入路径**，不冒充同账户任意代码的 OS 级权限限制。候选标记或 HKCU 登记本身不是产品操作权限；正式安装/更新域不调用候选验签器，缺 Owner 签名就拒绝。即使候选密钥持有者用通用签名算法签任意字节，**正式发行验证器也只认 Owner 公钥和正式清单格式**；“只能签候选”由受信 CI workflow 的输入约束、签名用途隔离和消费端拒绝共同实现，不谎称私钥的数学运算本身只能处理某类文本。候选密钥的生成与一次性 CI secret 配置由 Controller 在本设计合并后的授权范围内处理；Owner 发行私钥永不进入 CI。候选公钥轮换会改变 exe 字节，必须走完整可执行文件更新。
...
`--uninstall` / `--uninstall --quiet` 在 Tauri/WebView 初始化之前分流，只允许从已登记的正式安装或**隔离候选安装**的 `gogoke.exe` 执行；便携副本、未知注册根或已改指向的路径拒绝。即使同在 Owner 账户下，候选与正式安装仍使用不同 HKCU 登记、安装/数据根和实例 ID，不能相互卸载。卸载、完整更新、资源更新与重装共用同一安装生命周期锁；主程序核当前 exe、对应 HKCU 登记、安装实例 ID 与拥有文件清单后，把锁和已核实例身份连续交给收尾，收尾持锁直到删除/失败回执落地。安装或登记若在交接中已变，收尾拒绝。所有待删路径、祖先和实际打开对象都不得有 reparse，用户数据根不在任何待删子树。产品数据、Project、Context、settings 和用户自行加入安装目录的文件不因卸载被递归删除。
...
R2-06 的必要证据为：精确 SHA 的双云端可执行文件逐字节比较；只改资源/版本时 shell 与 host 哈希保持；CI 候选签名的实产品启动、候选/正式安装与数据根分离、正式域拒绝候选签名；受管测试账本写入及回读仍绑定固定仓库、分支和路径，拒绝该通道指向非测试 ref；Owner 正式签名后的相同 exe/installer 字节启动与篡改拒绝；云端正式安装和 `--uninstall`/数据保留，并发更新/重装时旧卸载不能删新实例；资源更新无安装程序进程、失败回滚与崩溃恢复；Owner Win11 的**正式安装包**实际产品入口、所需每个子进程与资源更新观察；独立复核。每项记录实际 run、进程、文件 hash、非零测试数、fail/skip。同账户候选根分离不能证明任意候选 JS 无法直接访问 Owner 数据或凭据；该轴不记 PASS。候选实测**不等待 Owner 签名**；云端 PASS、便携运行、单个旧版本放行或候选签名都不代替正式发行签名和最终 Owner Win11 证据。R2-05 仍待它自己的 Goal/结果链、PR/接受事实回读和最终审查；PR #34 在本设计 PR 中保持打开、不合并。
