...
启动顺序固定为：解析实际安装/便携资源根与明确的运行域 → 读取该域的签名清单和 `resource-index.json` → 用**编译期且相互独立的公钥**验签 → 检查清单用途、版本、索引 SHA-256 与运行域一致 → 从索引核对当前 shell、native-host、Node 与资源代的路径/长度/hash → 才注册只服务该已核资源代的本地 custom protocol 并创建主 WebView → 才准启动 T3 服务及 native-host。正式发行/更新域**只**接受 Owner 签署的 `SHA256SUMS.windows`；隔离候选域只接受下文 CI 签署、明确 `CI_CANDIDATE_RESOURCE` 用途的候选资源清单，并要求云端构建的 NSIS 已登记安装实例，拒绝便携 ZIP、工作树及仅靠命令行指定的候选根。两个验签入口不互相回退。签名、索引、文件或路径任何一步不成立，就不加载前端、不建立产品任务入口，保留有界错误证据。公钥、允许主机及更新协调器仍是编译期常量，不从资源、配置、Context 或数据库导入。
...
候选公钥与 Owner 公钥都固定编进**同一份**稳定 shell；候选 sidecar 只在显式隔离的候选安装/测试域验签。**资源包中的服务 JS 是可执行代码**，可直接发网络请求或读当前用户文件；native Product Authority 只约束它受管的入口，不能充当 OS 沙箱。候选实测必须在无管理员权限、无正式产品数据访问权的独立 Windows 标准用户或一次性云端测试主体下运行；安装根、HKCU、AppData 与所有子进程均属于该主体。候选验签入口在加载任何资源前核 OS token SID、由管理员控制且候选用户不可写的预注册测试身份、安装实例 ID 与根 ACL；任一不符即拒绝，不能靠命令行开关进入。启动器不继承 Owner 的环境、`GH_TOKEN`、`GITHUB_TOKEN`、`gh` 登录、凭据管理器或可读的生产密钥，OS ACL 必须实际拒绝访问 Owner 资料；候选 signer job 不运行候选代码，普通候选测试 job 不持有仓库内容写入凭据。候选产品域只做无凭据的资源/安装/更新/卸载实测及公开仓只读，不承担 R2-05 的 Git 写入；若将来需要候选写入已授权测试 ledger 分支，必须另有**候选进程拿不到原始凭据**、在进程外强制固定仓库/分支/操作的权限通道，经设计复核后才启用，不能沿用 Owner 的 `gh` 登录。候选标记、命令行开关或 HKCU 登记本身都不是权限；native Product Authority 仍核测试身份，测试入口拒绝正式更新、生产事实采用和 Goal Acceptance。正式安装/更新域不调用候选验签器；缺 Owner 签名就拒绝。即使候选密钥持有者用通用签名算法签任意字节，**正式发行验证器也只认 Owner 公钥和正式清单格式**；“只能签候选”由受信 CI workflow 的输入约束、签名用途隔离和消费端拒绝共同实现，不谎称私钥的数学运算本身只能处理某类文本。候选密钥的生成与一次性 CI secret 配置由 Controller 在本设计合并后的授权范围内处理；Owner 发行私钥永不进入 CI。候选公钥轮换会改变 exe 字节，必须走完整可执行文件更新。

候选使用与最终正式发行**同一冻结** `setup.exe`、`gogoke.exe`、native-host 和资源包字节；区别仅是外部候选 sidecar 与运行域。运行候选安装程序前还须以可信 GitHub run/artifact 回读的精确 hash 核本地 `setup.exe`，候选资源签名不能代替安装程序来源核验；安装后再核实际 exe 字节。CI 与已获授权的 Owner Win11 只能在上述独立测试主体中运行**云端构建、经 NSIS 实际安装**的产品，先机械核安装登记/实例 ID、主体 SID、无正式数据/凭据可达性和 SAC 实际强制状态；任一不成立，该环境的候选轴记 `NOT_RUN`，不得改用便携包、工作树或 Owner 正式登录主体试跑。无需 Owner 为每个候选签名、手动转交产物或点击测试控件。候选执行仍只是 `TEST_ONLY` 平台证据，不是正式 release、adoption 或 R2-05 最终验收。
...
`--uninstall` / `--uninstall --quiet` 在 Tauri/WebView 初始化之前分流，只允许从已登记的正式安装或**隔离候选安装**的 `gogoke.exe` 执行；便携副本、未知注册根或已改指向的路径拒绝。候选与正式安装使用不同登记和数据根，不能相互卸载。卸载、完整更新、资源更新与重装共用同一安装生命周期锁；主程序核当前 exe、对应 HKCU 登记、安装实例 ID 与拥有文件清单后，把锁和已核实例身份连续交给收尾，收尾持锁直到删除/失败回执落地。安装或登记若在交接中已变，收尾拒绝。所有待删路径、祖先和实际打开对象都不得有 reparse，用户数据根不在任何待删子树。产品数据、Project、Context、settings 和用户自行加入安装目录的文件不因卸载被递归删除。

主程序不能删除仍映射的自身；它完成验证后只启动**固定的系统工具**执行受限收尾，确认收尾已持同一生命周期锁和实例身份，再等待主进程实际退出。收尾删除列明的产品文件、属于本产品的快捷方式与 HKCU 卸载登记，写有界结果回执；每一步实际删除前须重核原安装实例及打开对象身份，采用不跟随 reparse 的删除，路径或对象已变就停止，不递归扫新目录。删除集合来自已验证的固定规则和安装文件清单，不执行可更新资源中的任意脚本；失败保留现场，不能以等待超时代替“文件已删除”的检查。云端并发烟测覆盖“核路径后重装/更新推进同名根”，证明旧收尾不删新实例。此形状借鉴 [Sandglass `uninstall.py` 的安装根/manifest/reparse 与状态目录检查](https://github.com/taiyun668/Sandglass/blob/bf043c8afbf96540e72cfa3ef2c072bb03d2c200/sandglass/uninstall.py#L124-L250)及其父进程退出后收尾；不直接复制 Python 实现。
...
R2-06 的必要证据为：精确 SHA 的双云端可执行文件逐字节比较；只改资源/版本时 shell 与 host 哈希保持；CI 候选签名的实产品启动、隔离与正式域拒绝，包括候选 JS 直接读正式资料、取正式凭据、写非测试 Git ref 的拒绝；Owner 正式签名后的相同 exe/installer 字节启动与篡改拒绝；云端正式安装和 `--uninstall`/数据保留，并发更新/重装时旧卸载不能删新实例；资源更新无安装程序进程、失败回滚与崩溃恢复；Owner Win11 的**正式安装包**实际产品入口、所需每个子进程与资源更新观察；独立复核。每项记录实际 run、进程、文件 hash、非零测试数、fail/skip。候选实测**不等待 Owner 签名**；云端 PASS、便携运行、单个旧版本放行或候选签名都不代替正式发行签名和最终 Owner Win11 证据。R2-05 仍待它自己的 Goal/结果链、PR/接受事实回读和最终审查；PR #34 在本设计 PR 中保持打开、不合并。
...
| 独立候选 Windows 测试主体 | **仅当创建标准用户、设置 OS ACL 或安全确认超出现有本机授权时一次性决定。** 这是把可执行候选代码与正式数据/凭据隔开的 OS 安全边界；不能用 Owner 当前登录身份的环境变量清理冒充隔离。 | 在获授权边界内配置独立低权限主体、安装与数据根；机械核 SID、访问拒绝、无 `gh` 登录和 SAC 强制状态，测试后清理。不能成立则记 `NOT_RUN`，不请求逐候选人工操作。 |
| 每个候选的构建、签资源、云端与 Win11 实测 | **无签名或手工测试触点。** CI 保管候选密钥；隔离主体与既有安装授权覆盖时由 Controller 执行，不让 Owner 点按钮或传截图。 | 无秘密构建 → 受信 CI 候选签名 → 精确候选测试 → 自动收集 SHA/run/进程/SAC 证据；普通失败修复与重测。 |
