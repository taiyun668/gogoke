...
| [AX（google/ax）](https://github.com/google/ax) | Apache 2.0，约 1.06 万星，v0.3.0（2026-09-20），自称接口仍会大改 | 集群级 agent 编排器，形如 Kubernetes：声明式 YAML 定义 `Task`（隔离沙箱，限 CPU/内存）、`Workspace`（预接 Git 仓库、MCP 服务、技能包，让 agent 热启动）、`Model`（平台用哪家模型，凭据来自 Kubernetes secret）；`ax apply / watch / ssh / suspend / resume`；状态存 Redis、事件流作工作队列、控制器水平扩展，底层沙箱为 Agent Substrate。模型走 API 凭据而非订阅 CLI；需 Kubernetes 集群；无事实账本、独立验收、分阶段放权、额度调度，只提"没人盯时会烧钱"而不管 | 项目层配置（声明式）；交接包与热启动；挂起/续接；远期云端沙箱工人池 | 用声明式清单描述任务、工作区、模型；工作区预接仓库/MCP/技能的热启动；挂起时保存状态、原样续接；随时 ssh 进沙箱的可交互接管 | 只借思路（不满足编排引擎一票否决：不驱动现成订阅 CLI，且依赖集群） |
