...
6. **额度与厂商的现实已经变了。**
   ...
   - [待] Anthropic 会按请求内容把模型静默降级：网络安全类转到 Opus 4.8，生物类转到 Opus 5（Reuters 2026-09-22 转述）。
...
- 自家三个项目（gogo-party、NaveHQ、Sandglass）的返工与停工记录，逐条列出根因。
- 零件的源码级拆解，先做执行底座：Orca、Paseo、OpenMausBot、Happier、Superset、Conductor 及 Microsoft Conductor、cli-agent-orchestrator。然后是额度通道、会话转移、记忆与复核。
- 本文所有 [待] 条目回到一手来源复读，尤其是 §2、§6、§7 里的具体数字与日期。
- 第 8 块（以后的业务领域）的外部调研。
