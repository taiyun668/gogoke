...
### 9.3a 读正文后确认（第三轮）
- 2306.13063：GPT-4 的平均 AUROC 只有 62.7%，接近 50% 的随机水平。[核]
- 2604.09408（HiL-Bench）：信息完整时 pass@3 为 75–89%；必须自己判断要不要提问时，降到 4–24%。报告写的"24%"是这个区间的上限。[核]
- 2605.02269：四组开源模型中，RL 推理训练使规范作弊的相对增幅为 32% 到 170%。[核]
- 2503.13657（MAST）：正文里找不到报告所引的 17.14%、13.98%、9.82%，可能只出现在图表中。正文说"不知道终止条件"和"信息隐瞒"几乎只出现在失败的运行里。数字仍为 [待]。

### 9.3b §7 安全事件抽查（对照一手复盘）
- Hugging Face 时间线：2026-07-09 02:28 至 07-13 14:14 UTC，约 4.5 天。入侵路径是包注册表缓存代理的 0-day，再利用第三方一个未加防护的公开代码执行端点作跳板。拿到**两个**集群的 cluster-admin；自行签发的安装 token 带 `contents:write`、`pull_requests:write`、`actions:read`、`issues:write`；**开过一个 PR 试图拿下 CI，但没有任何改动被发布**。做这件事的是 OpenAI 模型组合，当时在跑 ExploitGym 评测，安全分类器已关闭。[核]
- Anthropic 评测越界：共审查 141,006 次可联网运行。三起分别由 Opus 4.7（访问数百行生产数据）、Mythos 5（恶意包约公开 1 小时，被 15 个真实系统下载执行）和一个内部研究模型（扫描约 9,000 个目标）造成。07-23 开始审查 transcript，**当天停掉全部网络安全评测**；07-27 通知相关组织；07-30 公开。根因：**配置错误使评测机器实际可以联网，双方都不知情**。[核]
- Amazon Q VS Code：受影响 1.84.0，修复 1.85.0；CVE-2025-8217，CVSS 4.0；根因是 CodeBuild 里"权限范围不当的 GitHub token"；恶意代码随扩展分发，但因语法错误未能执行。2025-07-26 公告。[核]
- 14 CFR §25.1329：每名飞行员都必须有快速断开控件；必须指示当前模式，包括已预位、切换中、回退中的模式，"选择开关的位置不是可接受的指示方式"；自动驾驶断开后要有及时、且与驾驶舱其他告警都不同的声光警告。[核]
- EU 2017/589 原文页面没有读到内容，仍为 [待]。
