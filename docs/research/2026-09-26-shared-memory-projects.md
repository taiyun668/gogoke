...
## 6. 四个动作：汇总、统一管理、分发、转译（2026-09-26）

Owner 把这一块归纳成四个动作。按动作对照现有的开源项目：

| 动作 | 做这件事的项目 | 许可和规模 |
|---|---|---|
| **汇总**：把各家的会话和过程收进来 | cass、Agent Sessions（读本机历史）；claude-mem（用钩子实时采集） | cass 有附加限制；Agent Sessions 是 MIT，878★；claude-mem 是 Apache，94.7k★ |
| **统一管理**：分作用域、去重、推翻、遗忘 | mem0（作用域）、Graphiti（有效期加出处）、ByteRover（上下文树、共享空间） | Apache；Apache；许可证待核 |
| **分发**：让各家 CLI 都能读到 | MCP 服务（supermemory、Basic Memory、各种 obsidian-mcp）；**规则分发：ruler（intellectronica/ruler，2.9k★，MIT）、rulesync（dyoshikawa/rulesync，1.5k★，MIT）**，把同一套规则写成每家 CLI 自己的指令文件格式 | —— |
| **转译**：转成目标 CLI 的原生格式 | Codex 官方的外部 agent 导入（Apache，Rust）、CASR、sessport | 见 §5 |

**没有一个项目把四个动作都做全，而且都不做项目隔离和权限管控。**所以仍然按"拼 + 补全"来做：核心是我们自己的席位账本和记忆存储，负责按作用域管理和权限；每个动作从上表里借。

ruler 和 rulesync 可以直接对应设计 37 里"席位模板 → 固定前缀"这一步：同一份职责定义，按绑定的实例渲染成 `AGENTS.md`、`CLAUDE.md`、`GEMINI.md` 等各家的格式。

### 能不能用 Obsidian

- **适合放**：面向人读的长期记忆、知识、秘书长的笔记。Obsidian 的库就是一个 Markdown 文件夹，你和 AI 都能读写，能用 Git 管理，能双链。这是很常见的做法：claude-obsidian 有 15k★，obsidian-mcp 类服务有好几个。Basic Memory 也是 Markdown 加 MCP（但它是 AGPL）。
- **不适合放**：席位账本的核心。原因有四：
  1. 账本是高频的结构化事件（工具调用、用量、状态），多个席位同时写，Markdown 文件扛不住，也不好对账；
  2. Obsidian 不做权限，任何能访问文件系统的 agent 都能读全库，守不住项目隔离；
  3. Obsidian 不负责转译和分发，只是一个查看、编辑的界面；
  4. Obsidian 应用本身不是开源的，虽然库是普通文件、不依赖它也能用。
- **Owner 2026-09-26：不用 Obsidian，用不到。**不做 Obsidian 兼容的输出。
- 原建议（已否）：账本用我们自己的结构化存储（设计 37 §3）；长期记忆和知识**按 Obsidian 兼容的 Markdown 格式输出**，放在按作用域分开的文件夹里。你想看、想改，就用 Obsidian 打开对应的文件夹，但它只是一个窗口，不是系统的一部分。
